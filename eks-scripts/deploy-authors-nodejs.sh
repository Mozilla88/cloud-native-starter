...
function setup() {
  ...
  docker build -f Dockerfile -t $REGISTRY/$REGISTRY_NAMESPACE/authors:1 .
  docker push $REGISTRY/$REGISTRY_NAMESPACE/authors:1
 
  _out Deploy to EKS
  cd ${root_folder}/authors-nodejs/deployment
  # Add ECR tags to K8s deployment.yaml  
  sed "s+authors:1+$REGISTRY/$REGISTRY_NAMESPACE/authors:1+g" deployment.yaml > EKS-deployment.yaml  
