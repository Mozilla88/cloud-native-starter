...
function setup() {
  ...
  docker build -f Dockerfile.nojava -t $REGISTRY/$REGISTRY_NAMESPACE/web-api:1 .
  docker push $REGISTRY/$REGISTRY_NAMESPACE/web-api:1
 
  kubectl apply -f deployment/kubernetes-service.yaml

  # Add ECR tags to K8s deployment.yaml  
  sed "s+web-api:1+$REGISTRY/$REGISTRY_NAMESPACE/web-api:1+g" deployment/kubernetes-deployment-v1.yaml > deployment/EKS-kubernetes-deployment-v1.yaml
  kubectl apply -f deployment/EKS-kubernetes-deployment-v1.yaml
